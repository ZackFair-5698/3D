...
function init() {
  ...
  const e_Geometry = new THREE.SphereGeometry( 300, 24, 24 );
  ...
  const e_materials = new THREE.MeshBasicMaterial( { color: 0xffffff, wireframe: true } );
  const e_box = new THREE.Mesh(e_Geometry, e_materials);
  scene.add(e_box);

  const m_Geometry = new THREE.SphereGeometry( 34, 8, 8 );
  ...
  const m_materials = new THREE.MeshBasicMaterial( { color: 0xCC0000, wireframe: true } );
